...
impl CoreState {
    /// Move mouse off-screen and trigger a hover recalculation to clear stale states
    fn invalidate_mouse(&mut self) {
        self.mouse_pos = (-1.0, -1.0);
        self.frame_mouse_pos = (-1.0, -1.0);
        self.mouse_moved = true;  // <-- Force one recalculation with off-screen position
            ...
            WindowEvent::MouseInput { state, button, .. } => {
                ...
                if state == ElementState::Pressed {
                    ...
                    if let Some(win) = self.core.window.as_ref() {
                        ...
                        // is in header? (vs not workflow distingstion)
