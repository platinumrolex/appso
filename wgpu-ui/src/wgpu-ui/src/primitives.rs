...
pub enum HoverEffect {
    ...
    Button {
        ...
        bg_pressed: [f32; 4],
        bg_active: [f32; 4],           // new
        bg_active_hover: [f32; 4],     // new
        text_idle: [f32; 4],
        text_hover: [f32; 4],
        text_active: [f32; 4],         // new
        text_active_hover: [f32; 4],   // new
        corner_radius: f32,
    },
    Highlight {
        bg_hover: [f32; 4],
...
impl HoverEffect {
    pub fn resolve_bg(&self, is_hovered: bool, is_pressed: bool, is_active: bool) -> Option<[f32; 4]> {
        match self {
            HoverEffect::None => None,
            HoverEffect::Button { 
                bg_idle, bg_hover, bg_pressed, 
                bg_active, bg_active_hover, 
                ..
            } => {
                if is_active {
                    Some(if is_hovered { *bg_active_hover } else { *bg_active })
                } else {
    ...
    pub fn resolve_text(&self, is_hovered: bool, is_active: bool) -> Option<[f32; 4]> {
        match self {
            HoverEffect::Button { 
                text_idle, text_hover, 
                text_active, text_active_hover, 
                ..
            } => {
                if is_active {
                    Some(if is_hovered { *text_active_hover } else { *text_active })
                } else {
